...
def send_login_details(user, user_password):
    """Once registered, users (teacher, manager, admin) will be notified via email"""
    send_email(
        '[Vortex] You have been registered!',
        sender=app.config['MAIL_DEFAULT_SENDER'],
        recipients=[user.email],
        text_body=render_template(
...
def request_account_deletion(admin, Employee):
    """Request to delete Employee account sent to all admins"""
    send_email(
        subject='[Vortex] Request to Deactivate Account',
        sender=app.config['MAIL_DEFAULT_SENDER'],
        recipients=[admin.email],
        text_body=render_template(
            '/emails/deactivate_account/Employee_email.txt',
            admin=admin,
            Employee=Employee),
        html_body=render_template(
            '/emails/deactivate_account/Employee_email.html',
            admin=admin,
            Employee=Employee))
